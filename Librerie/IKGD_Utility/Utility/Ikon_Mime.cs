...
namespace Ikon.Mime
{
  ...
  public sealed class MimeExtensionHelper
  {
    ...
    public static string FindExtensionWithPoint(string mimeType)
    {
      return ExtensionTypes.GetExtensionWithPoint(mimeType);
    }

    public static IEnumerable<string> FindExtensions(string mimeType)
    {
      return ExtensionTypes.GetExtensions(mimeType);
    }

    public static IEnumerable<string> FindExtensionsWithPoint(string mimeType)
    {
      return ExtensionTypes.GetExtensionsWithPoint(mimeType);
    }
    ...
    {
      string extension = Utility.PathGetExtensionSanitized(file).TrimSafe('.', ' ');
    ...
    public static string FindMimeByData(byte[] dataBytes, string mimeProposed)
    {
      ...
      int result = FindMimeFromData(IntPtr.Zero, null, dataBytes, dataBytes.Length, String.IsNullOrEmpty(mimeProposed) ? null : mimeProposed, 0, out outPtr, 0);
      if (result != 0)
    ...
    private class MimeTypeCollection : List<MimeTypeCollection.mimeTypeInfo>
    {
      ...
      private void Init()
      {
        if (_extensions == null || _mimes == null || _extensions.Count == 0 || _mimes.Count == 0)
        {
          try
          {
            ...
            {
              if (_mimes.ContainsKey(mime.MimeType))
              {
                foreach (string ext in mime.Extensions)
                  if (!_mimes[mime.MimeType].Contains(ext))
                    _mimes[mime.MimeType].Add(ext);
                foreach (string ext in mime.Extensions)
      ...
      public string GetExtensionWithPoint(string type)
      {
        Init();
        return _mimes.ContainsKey(type) ? "." + _mimes[type][0] : string.Empty;
      }

      public IEnumerable<string> GetExtensions(string type)
      {
        Init();
        return _mimes.ContainsKey(type) ? _mimes[type].AsEnumerable() : Enumerable.Empty<string>();
      }

      public IEnumerable<string> GetExtensionsWithPoint(string type)
      {
        Init();
        return _mimes.ContainsKey(type) ? _mimes[type].Select(m => "." + m) : Enumerable.Empty<string>();
      }
      ...
      {
        string mimeDB = @"application/andrew-inset			ez
...
";
        foreach (var record in Utility.Explode(mimeDB, "\n", "\n\r\t ", true))
        {
          var frags = Utility.Explode(record, " \t", " \t", true);
          if (frags.Count >= 2)
            this.Add(new mimeTypeInfo(frags.FirstOrDefault(), frags.Skip(1).ToList()));
        }
            ...
            foreach (string keyname in subKeyNames)
            {
              ...
              using (RegistryKey curKey = classesRoot.OpenSubKey(subKey))
              {
                ...
                if (extension.Length > 0)
                  this.Add(new mimeTypeInfo(trimmed, new List<string>(new[] { extension })));
              }
            }
          }
        }
